...
export class CommentComponent implements OnInit {
  ...
  @Output('removeCommentOut') removeCommentOut = new EventEmitter<string>();
  ...
  getMember(): void {
    ...
    if (this.userName)
      this._memberService.getByUserName(this.userName)
        .pipe(
          take(1))
        .subscribe({
  ...
  remove(): void {
    if (this.memberInput)
      this._commentService.remove(this.memberInput.userName).pipe(
        take(1))
        .subscribe({
          next: (res: ApiResponse) => {
            this.getComments();
